...
const CaseStudyPage = ({ post }: CaseStudyPageProps) => {
  // Extract case study data based on post ID
  const getCaseStudyData = () => {
    switch (post.id) {
      case 'audio-visual-equipment-wholesaler':
        return {
      ...
      case 'multi-national-specialty-insurance':
        return {
      ...
      case 'saas-healthcare-achieves-remarkable-insights':
        return {
      ...
      case 'upscale-home-improvement-goods-manufacturer':
        return {
  ...
  const caseData = getCaseStudyData();
