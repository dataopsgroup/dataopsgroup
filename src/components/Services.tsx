...
const Services = () => {
  return <section id="services" className="py-16 md:py-24 px-4 md:px-8 bg-gray-50">
      <div className="container mx-auto">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {services.map((service, index) => <div key={index} className="bg-white p-8 rounded-lg shadow-sm hover:shadow-md transition-shadow">
              ...
              <a href={service.href} className="text-dataops-600 hover:text-dataops-700 font-medium inline-flex items-center transition-colors" aria-label={`Learn more about ${service.title}`}>
                Learn More →
              </a>
            </div>)}
        </div>
